...
PROJECT_NAME=plone-vercel-test
STACK_NAME=plone-vercel-test-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://plone-vercel-test.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend --build-arg PLONE_VERSION=${PLONE_VERSION} -t collective/plone-vercel-test-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend --build-arg VOLTO_VERSION=${VOLTO_VERSION} -t collective/plone-vercel-test-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name plone-vercel-test-backend-acceptance -d collective/plone-vercel-test-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name plone-vercel-test-frontend-acceptance --link plone-vercel-test-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/plone-vercel-test-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop plone-vercel-test-frontend-acceptance
	@docker stop plone-vercel-test-backend-acceptance
